...
describe('PATCH id /tasks/:id', () => {
  // Test to see if we are unable to update the task id manually
  test('It should respond with a 200 where the task fields update except for the id', async () => {
    const response1 = await request(app)
      .put('/tasks/2')
      .type('form')
      .send({
    ...
    // Should respond with fully updated task but id should remain the same
    expect(response1.body).toEqual(
      {
        id: 2,
        title: 'Brush Teeth',
